...
@AllArgsConstructor
public class SurnameRegisterState implements SessionState<RegisterStateData> {
    ...
    public boolean handle(Update update, TelegramClient client, SessionWithState<RegisterStateData> session) throws TelegramApiException {
        session.getData().setSurname(update.getMessage().getText().trim());
        session.updateState(new SportCategoryRegisterState());
        return false;
    }
}
